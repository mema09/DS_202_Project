...
min_ord = ggplot(min,aes(State.or.other...jurisdiction,X2018))+geom_point()+theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 4))+labs(title = "State Minimum Wage in 2018", x="State", y="Minimum Wage (USD)")
ggplotly(min_ord)
```

Is there a correlation with tuition and wage over time?
To take a look at the coorelation, first we looked at the trend for tuition over the years. As seen below, over time, tuition increases at a steady rate.
```{r}
...
Next we took a look at the trend for minimum wage over the years. As seen below, over time, tuition increases at semi-steady rate.
```{r}
new=min %>% 
